...
function extractFieldNames(schemaContent: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (fieldMatch?.[1]) {
      ...
      const isOptional =
        line.includes(".optional()") || line.includes(".nullable()");
...
export function generateToolMetadata(
  options: GenerateToolMetadataOptions,
): void {
  ...
  if (docsOutputDir) {
    const indexContent =
      ...
      allServices
        .map((s) => `- [${s.className}](./${s.className}.md)`)
        .join("\n") +
      `\n\n---\n\n*Generated by quickdraw-core*\n`;
  ...
  const totalMethods = allServices.reduce(
